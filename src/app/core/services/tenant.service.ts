...
export class TenantService {
	...
	private resolveSubdomain(subdomain: string | null): Promise<string> {
		...
		return firstValueFrom(
			this.#http.post<TenantResolveResponse>(
				`${environment.apiUrl}${TENANT_RESOLVE_ENDPOINT}`,
				{ domain: subdomain } satisfies TenantResolveBody
			)
		).then((res) => {
	...
	async init(): Promise<void> {
		...
		try {
			...
			console.error(`[TenantService] Could not resolve subdomain '${subdomain}':`, error);
